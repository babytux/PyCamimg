...
class camimgplugin(ICamimgPlugin):
    ...
    def getName(self):
        ...
        return gettext.translation(camimgpluginName, __LOCALE_FOLDER__,
                                   languages=[__LANGKEY__], fallback=True).gettext("Google Core")
    ...
    def showPluginConfiguration(self, parent=None):
        """
        @summary: Shows pl ugin configuration dialog.
        @param parent: Parent window. 
        """
        from AuthDialog import AuthDialog
        dialog = AuthDialog(parent=parent)
        dialog.run()
        del dialog
        dialog = None
