...
import GrayScaleOperation
from GrayScaleOperation import Operation

import gettext
from GrayScaleOperation import camimgplugin
    

class CamimgPlugin(IOperationPlugin.OperationPlugin):
    """
    @summary: Defines grayscale operation.
    """
    def __init__(self):
        """
        @summary: Creates new grayscale operation plugin.
        """
        self.__gtkAction__ = gtk.Action("GrayScaleAction", self.__trans__("GrayScale Images"), self.__trans__("GrayScale selected images"), None)
    ...
    def callbackAction(self, action, currentTab, userData = None):
        ...
        """
        if (action.get_name() != "GrayScaleAction"):
    ...
    def __doGrayscale__(self):
        """
        @summary: Do sepia toning operation
        """
        if (self.__currentTab__ != None):
            params = {}
            grayscaleThread = Thread(target=self.__currentTab__.doOperation, args=(Operation.OPERATION, params, self.__trans__("Applying grayscale..."),))
            grayscaleThread.start()
            
            __log__.debug("GrayScale thread started. %s" % grayscaleThread)
        else:
    ...
    def getIconsActions(self):
        ...
        """
        return {"/MenuPyCaming/ToolsMenu/Operations/OperationsMenuAdditions/GrayScale": "grayscale.png",
                "/ToolsPyCamimg/ActionsToolItems/GrayScale": "grayscale.png"}
